...
This page provides quick navigation targets for all ``Molecule`` methods,
attributes, helper classes and helper functions mentioned in the API guide.
Each entry is a manually declared Sphinx signature — no source-code docstrings are
