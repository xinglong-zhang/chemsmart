...
**PBS/Torque**, or **LSF**. Other schedulers must submit child jobs individually. For CLI flags (``--run-in-parallel``,
``-M`` / ``--max-tasks``, ``--test``), see :ref:`cli-batch-array-submission`.
