...
This page documents the CLI options available for all Gaussian jobs.
Use ``chemsmart sub gaussian --help`` for the complete list.
...
.. note::
   ...
   - ``-f`` accepts various formats: ``.xyz``, ``.com``, ``.gjf``, ``.log``,
     ``.inp``, or ``.out``.
...
.. warning::
   ...
   If the input file lacks charge/multiplicity information, you must specify
   them with ``-c`` and ``-m``.
...
.. tip::
   ...
   This is useful when using an optimized neutral closed-shell structure to
   run calculations on a charged radical ion.
