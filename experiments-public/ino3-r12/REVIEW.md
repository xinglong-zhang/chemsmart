...
evidence it left behind. Raw evidence: `ino3-r12/workspace/`
(moved here unmodified; a symlink at the original campaign path keeps every
absolute reference in the record resolvable). Figures: `ino3-r12/figures/`.
...
The task (`workspace/.chemsmart-agent/goals/goal-ino3-r12/task.md`,
...
That rejection is right, and the same wording cuts the other way: if B3LYP and TPSSh are "vertical single
...
| what | where (relative to `ino3-r12/`) |
|---|---|
| task text | `workspace/.chemsmart-agent/goals/goal-ino3-r12/task.md` |
...
| the three sessions' public transcripts | `workspace/.chemsmart-agent/runs/live-*/public-transcript-*.json` |
| the seven executed nodes (`.inp`, `.out`, receipts) | `workspace/nodes/` |
| the two promoted geometries | `workspace/.chemsmart-agent/runs/live-20260909T0859*/artifacts/` |
...
provider profile `alibaba-token-plan-qwen`; harness `d692818c`.
