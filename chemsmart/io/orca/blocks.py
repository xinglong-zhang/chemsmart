...
class SCFORCABlock(ORCABlock):
    ...
    def write(
        ...
        # Flip spin is a vector and you can give a list
        # of atom on which you want to have the spin
        # flipped.  For example
        # FlipSpin 17,38,56
        # REMEMBER: counting starts at atom 0!
        finalms=None,  # FinalMs 0.0
        # The desired Ms value of the broken symmetry determinant.
        # This value MUST be given since the
        # program cannot determine it by itself
    ):
        """
    ...
    def write(  # noqa: PLR0912
        ...
        Arguments:
            ...
            citype: options:
                        ...
                        CEPA_1 # coupled-electron
                        pair approximation '1'
                        CEPA_2 # coupled-electron
                        pair approximation '2'
                        CEPA_3 # coupled-electron
                        pair approximation '3'
                        CPF_1 # Coupled-pair
                        functional approximation '1'
                        CPF_2           # (note that CPF/1 is identical with
                        CPF_3           #  the original CPF of Ahlrichs et al.)
                        VCEPA_1         # Variational CEPA approximation '1'
                        VCEPA_2         #
                        VCEPA_3         #
                        NCEPA_1 # our slightly
                        modified versions of CEPA
                        NCEPA_2         # and CPF
                        NCEPA_3         #
                        NCPF_1          #
                        NCPF_2          #
                        ...
                        SEOI # a strictly size extensive energy
                        functional maintaining unitary invariance
                                          (not yet available for UHF)
                        MP3 # MP3 calculation. With
                        UseSCS=true it is SCS-MP3
            ewin: -3,1e3 # orbital energy window to
            determine which MOs are included in the treatment
            singles: options:
                        true # include single excitations
                        in the treatment (default true)
                        false
            triples: options:
                        0 # (T) correction in
                        CCSD(T)/QCISD(T) default is no triples
                        1               # Algorithm 1 (lots of memory, fast)
                        2 # Algorithm 2 (less
                        memory, about 2x slower)
            brueckner: options:
                        true # use Brueckner
                        orbitals (default false)
                        false
            denmat: options:
                        none            # no evaluation of density matrix
                        linearized # density matrix obtained by
                        retaining only CEPA_0-like terms, i.e.,
                                        # those linear in the
                                        # excitation amplitudes
                        unrelaxed # unrelaxed density matrices, i.e.,
                        density matrices without orbital relaxation
                        orbopt # perform orbital optimization
                        yielding fully relaxed density matrices
                                        # (if citype chosen as
                                        # CCSD or QCISD this option
                                        # implies evaluation of the Z
                                          vector).
                                        # (default: linearized)
            zsimple: options:
                        true # simplified evaluation of the Z
                        vector in case of orbital optimized CCD
                                        # (citype chosen as CCSD or
                                        # QCISD and Denmat as orbopt)
                                        # by using an analytical
                                           formula
                        false # explicit solution of Z vector
                        equations in case of orbital optimized CCD
                                        # (default: false)
            useqros: # use of quasi-restricted
            orbitals (default false)
            localize: options:
                        0 # use localized MOs. Presently very
                        little use is made of locality. It may help
            ...
            natorbiters: options:
                        0 # Perform natural orbital iterations.
                        default is none. Not possible for CCSD
                                          and QCISD
            pccsdab: # the three
            parameters for parametrized
            pccsdcd: # coupled-cluster (default is
            1.0 which corresponds to normal CCSD)
            pccsdef: # this defines how the
            rate limiting step is handled
            ...
            kcopt: options:
                    ...
                    KC_AOX # Do it from stored
                    AO exchange integrals
            printLevel: options:
                    2                   # Control the amount of output.
                    3 # For 3 and higher things like pair
                    correlation energies are printed.
            maxiter: 35                  # Max. number of iterations
            trafotype: options: # How the
            integral transformation is done.
                                        # Note that it is fine to do AOX or
                                        # AO or AOBLAS together with trafo_ri
                    trafo_jk            # Partial trafo to J+K operators
                    trafo_ri # RI transformation of all integrals
                    up to 2-externals (3-ext for (T)) and
                                          rest on the fly
                    trafo_full # Full four index transformation.
                    Automatically chosen for KCOpt=KC_MO
            maxcore: 350 # Memory in MB - used for
            integral trafos and batching and for storage of
                                        # integrals and amplitudes
                                        # # don't be too generous
            stol: 1e-5 # Max. element of the
            residual vector for convergence check
            lshift: 0.3 # Level shift to be
            used in update of coefficients
            maxdiis: 7 # Max number of
            DIIS vectors to be stored.
