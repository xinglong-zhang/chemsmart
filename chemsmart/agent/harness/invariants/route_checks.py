...
def _issue(rule_id: str, severity: str, message: str, evidence: dict[str, Any]) -> InvariantIssue:
    return InvariantIssue(rule_id=rule_id, severity=severity, message=message, evidence=evidence)


def _result(rule_id: str, issues: list[InvariantIssue], context: dict[str, Any]) -> InvariantResult:
    verdict = "reject" if any(i.severity == "reject" for i in issues) else (
        "warn" if issues else "ok"
    )
    return InvariantResult(rule_id=rule_id, verdict=verdict, issues=tuple(issues), context=context)


def check_orca_ts_route(
    route: str | None, *, inputfile: str | None = None, result_index: int | None = None
) -> InvariantResult:
    rule_id = "orca.ts.route"
    ev = {"inputfile": inputfile, "result_index": result_index, "route": route}
    issues: list[InvariantIssue] = []
    if route is None:
        issues.append(_issue(rule_id, "reject", "ORCA TS route line is missing", ev))
    ...
    if n_optts == 0 and n_scants == 0:
        issues.append(_issue(rule_id, "reject", "ORCA TS route missing OptTS/ScanTS keyword", ev))
    if n_optts > 1 or n_scants > 1:
        issues.append(_issue(rule_id, "reject", "ORCA TS route contains a duplicate TS keyword",
                             {**ev, "optts_count": n_optts, "scants_count": n_scants}))
    return _result(rule_id, issues, ev)


def check_freq_route(
    route: str | None, *, software: str, inputfile: str | None = None,
    ...
    ev = {"inputfile": inputfile, "result_index": result_index, "route": route, "software": software}
    issues: list[InvariantIssue] = []
    if route is None:
        issues.append(_issue(rule_id, "reject", "route line is missing for a frequency job", ev))
        return _result(rule_id, issues, ev)
    ...
    if n == 0:
        issues.append(_issue(
            ...
            "(it will silently run without frequencies)", ev))
    elif n > 1:
        issues.append(_issue(rule_id, "reject",
...
def check_irc_route(
    ...
    issues: list[InvariantIssue] = []
    if route is None:
        issues.append(_issue(rule_id, "reject", "route line is missing for an IRC job", ev))
        return _result(rule_id, issues, ev)
