...
def click_orca_settings_options(f):
    ...
    @click.option('-f', '--filename', type=str, default=None, help='filename from which new ORCA input is prepared.')
    @click.option(
    ...
    @click.option('-i', '--index', type=str, default='-1', help='index of atom to use; default is last atom')
...
def orca(  # noqa: PLR0912, PLR0915
    ...
    if label is not None and append_label is not None:
        raise ValueError('Only give ORCA input filename or name to be be appended, but not both!')
