...
def click_gaussian_options(f):
    ...
    @click.option(
        '--scratch/--no-scratch', type=bool, default=True, help='To run in scratch or without scratch folder.'
    )
    @functools.wraps(f)
...
def click_gaussian_settings_options(f):
    ...
    @click.option(
        '-f', '--filename', type=str, default=None, help='filename from which new Gaussian input is prepared.'
    )
    @click.option(
        '-l', '--label', type=str, default=None, help='write user input filename for the job (without extension)'
    )
    @click.option('-a', '--append-label', type=str, default=None, help='name to be appended to file for the job')
    ...
    @click.option('-m', '--multiplicity', type=int, default=None, help='multiplicity of the atoms')
    @click.option('-x', '--functional', type=str, default=None, help='New functional to run.')
    @click.option('-b', '--basis', type=str, default=None, help='New basis set to run.')
    @click.option('-i', '--index', type=str, default=None, help='index of atom of ase db to be used')
    @click.option('-o', '--additional-opt-options', type=str, default=None, help='additional opt options')
    @click.option('-r', '--additional-route-parameters', type=str, default=None, help='additional route parameters')
    @click.option(
        '-A',
...
def gaussian(  # noqa: PLR0912, PLR0915
    ...
    if filename is None:
        ...
        logger.info(f'No filename is supplied and Gaussian default settings are used:\n{job_settings.__dict__} ')
        ...
        raise ValueError(f'Unrecognised filetype {filename} to obtain GaussianJobSettings')

    # Update keywords
    keywords = ('charge', 'multiplicity')  # default keywords to merge filename charge and multiplicity
    ...
    if filename is None and pubchem is None:
        raise ValueError('[filename] or [pubchem] has not been specified!\nPlease specify one of them!')
    if filename and pubchem:
        raise ValueError('Both [filename] and [pubchem] have been specified!\nPlease specify only one of them.')
    ...
    if label is not None and append_label is not None:
        raise ValueError('Only give Gaussian input filename or name to be be appended, but not both!')
    ...
    ctx.obj['atoms'] = atoms  # atoms as a list as some jobs requires all structures to be used
    ctx.obj['label'] = label
