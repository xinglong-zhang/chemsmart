...
def get_thermo(f, fs, fh, concentration, temperature, alpha, weight, q, qs, qh, filenames):
    ...
    if len(files) == 0:
        logger.info("Please provide calculation output files on the command line.")
        logger.info("Try 'get_thermochemistry.py --help' for help.")
        return

    logger.info(f"   Temperature = {temperature:.2f} Kelvin   Concentration = {concentration:.1f} mol/L")
    logger.info("   All energetic values below shown in Hartree unless otherwise specified.")
    logger.info("")
    if q or qs or qh:
        logger.info(
        ...
        logger.info("   REF: Chai, J.-D.; Head-Gordon, M. Phys. Chem. Chem. Phys. 2008, 10, 6615–6620")
        logger.info("")
    if q or qs:
        logger.info(
            f"   Entropic quasi-harmonic treatment: frequency cut-off value of {fs:.1f} wavenumbers will be applied.")
        logger.info("   QS = Grimme: Using a mixture of RRHO and Free-rotor vibrational entropies.")
    ...
    if q or qh:
        ...
        logger.info("   QH = Head-Gordon: Using an RRHO treatement with an approximation term for vibrational energy.")
        logger.info(
            "   REF: Li, Y.; Gomes, J.; Sharada, S. M.; Bell, A. T.; Head-Gordon, M. J. Phys. Chem. C 2015, 119, 1840-1850")
        logger.info("")

    if q:
        logger.info('   {:<39} {:>13} {:>10} {:>13} {:>13} {:>10} {:>10} {:>13} {:>13}'.format(
            "Structure", "E", "ZPE", "H", "qh-H", "T.S", "T.qh-S", "G(T)", "qh-G(T)")
        )
    elif qs:
        logger.info('   {:<39} {:>13} {:>10} {:>13} {:>10} {:>10} {:>13} {:>13}'.format(
            "Structure", "E", "ZPE", "H", "T.S", "T.qh-S", "G(T)", "qh-G(T)")
        )
    elif qh:
        logger.info('   {:<39} {:>13} {:>10} {:>13} {:>13} {:>10} {:>13} {:>13}'.format(
            "Structure", "E", "ZPE", "H", "qh-H", "T.S", "G(T)", "qh-G(T)")
        )
    else:
        logger.info('   {:<39} {:>13} {:>10} {:>13} {:>10} {:>13}'.format(
    ...
    for file in files:
        ...
        qrrho_entropy_times_temperature = thermochemistry.qrrho_entropy_times_temperature
        gibbs_free_energy = thermochemistry.gibbs_free_energy
        if q:
            qrrho_gibbs_free_energy=thermochemistry.qrrho_gibbs_free_energy
        elif qh:
            qrrho_gibbs_free_energy=thermochemistry.qrrho_gibbs_free_energy_qh
        elif qs:
            qrrho_gibbs_free_energy=thermochemistry.qrrho_gibbs_free_energy_qs

        if q:
            logger.info('{:2} {:39} {:13.6f} {:10.6f} {:13.6f} {:13.6f} {:10.6f} {:10.6f} {:13.6f} {:13.6f}'.format(
                "o", structure, energy, zero_point_energy, enthalpy, qrrho_enthalpy, entropy_times_temperature, qrrho_entropy_times_temperature, gibbs_free_energy, qrrho_gibbs_free_energy)
            )
        elif qs:
            logger.info('{:2} {:39} {:13.6f} {:10.6f} {:13.6f} {:10.6f} {:10.6f} {:13.6f} {:13.6f}'.format(
                "o", structure, energy, zero_point_energy, enthalpy, entropy_times_temperature,
                qrrho_entropy_times_temperature, gibbs_free_energy, qrrho_gibbs_free_energy)
            )
        elif qh:
            logger.info('{:2} {:39} {:13.6f} {:10.6f} {:13.6f} {:13.6f} {:10.6f} {:13.6f} {:13.6f}'.format(
                "o", structure, energy, zero_point_energy, enthalpy, qrrho_enthalpy, entropy_times_temperature, gibbs_free_energy, qrrho_gibbs_free_energy)
            )
        else:
            logger.info('{:2} {:39} {:13.6f} {:10.6f} {:13.6f} {:10.6f} {:13.6f}'.format(
                "o", structure, energy, zero_point_energy, enthalpy, entropy_times_temperature, gibbs_free_energy)
            )
