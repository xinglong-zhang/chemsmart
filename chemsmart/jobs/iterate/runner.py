...
def _run_combination_worker(combination: IterateCombination) -> tuple[str, Optional[Molecule]]:
    """
...
class IterateJobRunner(JobRunner):
    ...
    def __init__(
        ...
        logger.debug(f"IterateJobRunner initialized")
    ...
    def run_single(self, combination: IterateCombination) -> tuple[str, Optional[Molecule]]:
        """
        ...
        """
        if self.fake:
            logger.info(f"Fake mode enabled, not actually running {combination.label}.")
    ...
    def run_combinations(
        ...
        """
        if self.fake:
            logger.info("Fake mode enabled, not actually running combinations.")
        ...
        logger.info(f"Running {len(combinations)} combination(s) with {nprocs} process(es), timeout={timeout}s")
        ...
        with ProcessPoolExecutor(max_workers=max_workers) as executor:
            ...
            for future in as_completed(future_to_comb, timeout=None):
                ...
                try:
                    ...
                    results_dict[label] = result[1]  # result is (label, molecule)
                    ...
                    logger.warning(f"Timeout ({timeout}s) for combination: {label}")
        ...
        results = [(comb.label, results_dict.get(comb.label)) for comb in combinations]
        
        # Log results summary
        successful = sum(1 for _, mol in results if mol is not None)
        logger.info(f"Completed: {successful}/{len(results)} molecules generated successfully")
        
        # Print summary of failed/timed out tasks
        if timed_out_labels:
            logger.warning(f"Timed out ({len(timed_out_labels)}): {', '.join(timed_out_labels)}")
        if failed_labels:
            logger.warning(f"Failed ({len(failed_labels)}): {', '.join(failed_labels)}")
    ...
    def _load_molecule(
        ...
        try:
            if file_path:
                ...
                logger.debug(f"Loaded {mol_type} '{label}' from file: {file_path}")
            elif smiles:
                molecule = Molecule.from_smiles(smiles)
                logger.debug(f"Loaded {mol_type} '{label}' from SMILES: {smiles}")
            elif pubchem:
                molecule = Molecule.from_pubchem(pubchem)
                logger.debug(f"Loaded {mol_type} '{label}' from PubChem: {pubchem}")
            else:
                logger.warning(
    ...
    def _generate_combinations(
        ...
        for skel_idx, skel_config in enumerate(skeleton_list):
            ...
            skel_link_indices = skel_config.get('link_index')  # list[int], 1-based
            skeleton_indices = skel_config.get('skeleton_indices')  # list[int] or None
            
            if not skel_link_indices:
                logger.warning(f"Skeleton '{skel_label}' has no link_index, skipping.")
                continue
            
            for sub_idx, sub_config in enumerate(substituent_list):
                substituent = self._load_molecule(sub_config, 'substituent', sub_idx)
                if substituent is None:
                    continue
                
                sub_label = sub_config.get('label') or f"substituent_{sub_idx + 1}"
                sub_link_index = sub_config.get('link_index')  # list[int], 1-based
                
                if not sub_link_index:
                    logger.warning(f"Substituent '{sub_label}' has no link_index, skipping.")
                    continue
    ...
    def _write_output_xyz(
        self, 
        results: list[tuple[str, Optional[Molecule]]], 
        outputfile: str
    ) -> None:
        """
    ...
    def run(
        ...
        **kwargs
        ...
        results = self.run_combinations(combinations, nprocs=job.nprocs, timeout=job.timeout)
        
        # Write output xyz file
        self._write_output_xyz(results, job.outputfile)
        
        logger.info(f"IterateJobRunner completed job")
