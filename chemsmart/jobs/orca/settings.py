...
class ORCAJobSettings(MolecularJobSettings):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self._validate_basis_specification()
    ...
    def _validate_basis_specification(self):
        """Ask the per-element basis owner whether this can be written.

        The rules live with the specification rather than here, because
        a basis is not an electronic-structure pairing and the recurrence
        of these defects came from interpreting the fields at each call
        site instead of once.
        """

        from chemsmart.io.orca.basis import ORCAPerElementBasis

        ORCAPerElementBasis.from_settings_fields(
            basis=self.basis,
