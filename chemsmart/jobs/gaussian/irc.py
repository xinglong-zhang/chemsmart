...
class GaussianIRCJob(GaussianJob):
    ...
    def _ircf_job(self):
        ...
        return GaussianGeneralJob(folder=self.folder, atoms=self.atoms, settings=ircf_settings, label=label)
    ...
    def _ircr_job(self):
        ...
        return GaussianGeneralJob(folder=self.folder, atoms=self.atoms, settings=ircr_settings, label=label)
    ...
    def _job_is_complete(self):
        ...
        return self._run_forward_is_complete() and self._run_reverse_is_complete()
    ...
    def backup_files(self, backup_chk=False):
        ...
        self.backup_file(self._ircf_job().outfile, folder=folder)
        self.backup_file(self._ircr_job().outfile, folder=folder)
        if backup_chk:
