...
from pyatoms.io.gaussian.gengenecp import GenGenECPSection
from pyatoms.jobs.settings import MolecularJobSettings, read_molecular_job_yaml
from pyatoms.utils.utils import file_cache, get_prepend_string_list_from_modred_free_format
...
gassian_additional_route_parameters = GaussianRefs().gaussian_additional_route_parameters
...
class GaussianJobSettings(MolecularJobSettings):
    def __init__(
        ...
        **kwargs,
        ...
        if gen_genecp is not None and '~' in gen_genecp:
            gen_genecp = os.path.expanduser(gen_genecp)
    ...
    def merge(self, other, keywords=('charge', 'multiplicity'), merge_all=False):
        ...
        if keywords is not None:
            other_dict = {k: other_dict[k] for k in keywords if k in other_dict}
    ...
    def from_comfile(cls, com_path):
        ...
        Args:
            com_path (str): file path of the .com file string to be supplied.
        """
        from pyatoms.io.gaussian.inputs import Gaussian16Input

        com_path = os.path.abspath(com_path)
        logger.info(f'Return Settings object from comfile: {com_path}')
        gaussian_settings_from_comfile = Gaussian16Input(comfile=com_path).read_settings()
        logger.info(f'with settings: {gaussian_settings_from_comfile.__dict__}')
    ...
    def from_inpfile(cls, inp_path):
        ...
        Args:
            inp_path (str): file path of the .inp file string to be supplied.
        """
        from pyatoms.io.orca.inputs import ORCAInput

        inp_path = os.path.abspath(inp_path)
        logger.info(f'Return Settings object from inp file: {inp_path}')
        orca_settings_from_inpfile = ORCAInput(inpfile=inp_path).read_settings()
        gaussian_default_settings = cls.default()
        gaussian_settings_from_inpfile = gaussian_default_settings.merge(orca_settings_from_inpfile, merge_all=True)
        logger.info(f'with settings: {gaussian_settings_from_inpfile.__dict__}')
        return gaussian_settings_from_inpfile

    @file_cache()
    @staticmethod
    def from_logfile(log_path, **kwargs):
        """Return Gaussian settings object from a given gaussian.log file.

        Args:
            log_path (str): file path of the .log file string to be supplied.
            kwargs (dict): additional keyword arguments to be supplied to Gaussian16Output.
        """
        log_path = os.path.abspath(log_path)
        from pyatoms.io.gaussian.outputs import Gaussian16Output, Gaussian16OutputWithPBC

        logger.info(f'Return Settings object from logfile: {log_path}')
        try:
            settings = Gaussian16Output(logfile=log_path, **kwargs).read_settings()
        except ValueError:
            settings = Gaussian16OutputWithPBC(logfile=log_path, **kwargs).read_settings()
    ...
    def write_gaussian_input_from_job(self, job, jobrunner, **kwargs):
        ...
        return self.write_gaussian_input(
            atoms=job.atoms, job_label=job.label, num_cores=num_cores, mem_gigs=mem_gigs, **kwargs
        )

    def write_gaussian_input(
        self, atoms, job_label=None, num_cores=20, mem_gigs=20, output_dir=None, make_dir_if_not_present=True
    ):
        ...
        """
        logger.info(f'Writing gaussian input file with settings: {self.__dict__}')
        ...
        assert job_label is not None, f'Basename of Gaussian file to be written {job_label} cannot be None!'
        ...
        with open(filepath, 'w') as f:  # will close() when we leave this block
            self._write_gaussian_header(f=f, num_cores=num_cores, mem_gigs=mem_gigs, job_label=job_label)
            ...
            self._append_custom_solvent_parameters(f)  # followed by user defined solvennt parameters
    ...
    def _write_route_section_from_user_input(self, f):
        ...
        if not route_string.startswith('#'):
            route_string = f'#{self.dieze_tag} {route_string}' if self.dieze_tag is not None else f'# {route_string}'
    ...
    def _get_route_string_from_jobtype(self, atoms):  # noqa: PLR0912, PLR0915
        ...
        if self.dieze_tag is not None:
            route_string += f'#{self.dieze_tag}'  # e.g. dieze_tag='p' to get '#p'
        else:
            route_string += '#'
        ...
        if self.additional_opt_options_in_route is not None:
            if self.job_type == 'opt':
                route_string += f' opt=({self.additional_opt_options_in_route})'
        ...
        if self.ab_initio is not None and self.functional is None:
            ...
            raise ValueError('Warning: Both ab initio and DFT functional are provided!')
        else:
            raise ValueError('Warning: Both ab initio and DFT functional are missing!')

        if 'gen' in self.basis:
            genecp_section = self.get_genecp_section(atoms=atoms)
        ...
        elif self.solvent_model is not None and self.solvent_id is not None:  # solvation is turned on
    ...
    def _write_charge_and_multiplicity(self, f):
        if self.charge is None or self.multiplicity is None:
            raise TypeError(f'Charge is {self.charge} and Multiplicity is {self.multiplicity}')
            ...
            for i in range(3):  # 3D PBC vectors
                if not all(pbc_vectors[i] == [0.0, 0.0, 0.0]):
                    string = '{:5} {:15.10f} {:15.10f} {:15.10f}\n'.format(
                        'TV', float(pbc_vectors[i][0]), float(pbc_vectors[i][1]), float(pbc_vectors[i][2])
                    )
                    f.write(string)
        f.write('\n')
    ...
    def _write_cartesian_coordinates(self, f, atoms):
        ...
        for i, (s, (x, y, z)) in enumerate(zip(atoms.symbols, atoms.positions, strict=False)):
    ...
    def _append_modredundant(self, f):
        if self.modred:
            if isinstance(self.modred, list):
                ...
                prepend_string_list = get_prepend_string_list_from_modred_free_format(input_modred=self.modred)
                ...
                coords_list = self.modred['coords']
                prepend_string_list = get_prepend_string_list_from_modred_free_format(input_modred=coords_list)
                for prepend_string in prepend_string_list:
                    f.write(f"{prepend_string} S {self.modred['num_steps']} {self.modred['step_size']}\n")
            f.write('\n')
    ...
    def _genecp_elements_specified(self):
        return self.heavy_elements is not None and self.heavy_elements_basis is not None

    @property
    def _genecp_file_specified(self):
        return self.gen_genecp is not None and os.path.exists(self.gen_genecp)

    def get_genecp_section(self, atoms):
        if self._genecp_elements_specified:
            ...
            heavy_elements_in_structure = self.prune_heavy_elements(atoms)

            genecp_section = GenGenECPSection.from_bse_api(
                light_elements=self.get_light_elements(atoms),
            ...
            logger.info(f'GENECP file specified: {self.gen_genecp}')
            # Method 2 for getting genecp:
            # Supplied path to genecp file
            genecp_section = GenGenECPSection.from_genecp_path(genecp_path=self.gen_genecp)
        else:
    ...
    def get_light_elements(self, atoms):
        ...
        from pyatoms.utils.periodictable import sort_list_of_elements_according_to_atomic_number

        sorted_light_elements_list = sort_list_of_elements_according_to_atomic_number(light_elements_list)
        logger.info(f'Light elements in structure: {sorted_light_elements_list}')
    ...
    def apply_on(self, job, jobrunner):
        ...
        if jobrunner.scratch and jobrunner.scratch_dir is not None and os.path.exists(jobrunner.scratch_dir):
            ...
            self.write_gaussian_input_from_job(output_dir=job_scratch_dir, job=job, jobrunner=jobrunner)
            scratch_inputfile = os.path.join(job_scratch_dir, f'{job.label}.com')
            assert os.path.exists(scratch_inputfile), f'inputfile {scratch_inputfile} is not found'
        elif jobrunner.scratch and not os.path.exists(jobrunner.scratch_dir):
            logger.info(f'{jobrunner.scratch_dir} does not exist! Running job in {job.folder}.')
            self.write_gaussian_input_from_job(output_dir=job.folder, job=job, jobrunner=jobrunner)
            inputfilename = os.path.basename(job.inputfile)  # job.inputfile is the full path to the inputfile
            runfolder_inputfile = os.path.join(job.folder, inputfilename)
            assert os.path.exists(runfolder_inputfile), f'inputfile {runfolder_inputfile} is not found'
        else:
...
class GaussianLinkJobSettings(GaussianJobSettings):
    def __init__(self, stable='opt', guess='mix', **kwargs):
        super().__init__(**kwargs)
        self.stable = stable
        self.guess = guess
    ...
    def _write_route_section(self, f, atoms):
        route_string = self._get_route_string_from_jobtype(atoms=atoms)
        # remove "opt and freq" from route string
...
class GaussianTDDFTJobSettings(GaussianJobSettings):
    def __init__(self, states='singlets', root=1, nstates=3, eqsolv=None, **kwargs):
        super().__init__(**kwargs)
        self.states = states
        self.root = root
