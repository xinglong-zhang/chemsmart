...
class GaussianInputWriter(InputWriter):
    ...
    def _write_charge_and_multiplicity_qmmm(self, f):
        """
        Write ONIOM charge and multiplicity specifications for QM/MM calculations.
        ...
        For 3-layer ONIOM (High:Medium:Low):
            charge_real_low multiplicity_real_low charge_int_med multiplicity_int_med
            charge_int_low multiplicity_int_low charge_model_high multiplicity_model_high
            charge_model_med multiplicity_model_med charge_model_low multiplicity_model_low

        Args:
            f (file): Open file object to write charge and multiplicity line to.

        Note:
            The charge and multiplicity values are generated by the settings object's
            charge_and_multiplicity_string property, which handles the ONIOM-specific
    ...
    def _write_route_string_qmmm(self, f):
        ...
        Generates and writes the complete route line (#-line) for ONIOM calculations,
        including job type, theory levels for each layer, and ONIOM-specific keywords.
        The route string is constructed to specify the multi-layer calculation setup.
