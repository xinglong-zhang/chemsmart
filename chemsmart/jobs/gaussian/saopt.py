...
from pyatoms.utils.grouper import StructuralSelfConsistentGrouper, StructuralSequentialGrouper
...
class GaussianSAOptJob(GaussianJob):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        last_num_structures = int(round(len(atoms) * proportion_structures_to_use, 1))
    ...
    def unique_structures_sequential_grouper(self):
        ...
        grouper = StructuralSequentialGrouper.from_atoms(
            atoms=images, num_procs=self.num_grouper_processes, skip_structure_reduction=True
        )
    ...
    def unique_structures_self_consistent_grouper(self):
        ...
        unique_images_energies = [image.get_potential_energy() for image in unique_images]
        unique_images_sorted = [unique_images[i] for i in np.argsort(unique_images_energies)]
    ...
    def unique_structures_energies(self):
        return [structure.get_potential_energy() for structure in self.unique_structures]
    ...
    def _prepare_all_jobs(self):
        jobs = []
        logger.info(f'Number of structures used for optimization: {self.num_unique_structures}\n')
        logger.info(f'Unique structures: {self.unique_structures}')
        logger.info(f'Unique structures energies: {self.unique_structures_energies}')
        for i in range(self.num_unique_structures):
            label = f'{self.label}_c{i + 1}'  # 1-indexed for structures
            jobs += [
                GaussianGeneralJob(
                    folder=self.folder, atoms=self.unique_structures[i], settings=self.settings, label=label
                )
            ]
        return jobs
    ...
    def incomplete_structure_opt_jobs(self):
        return [job for job in self.all_structures_opt_jobs if not job.is_complete()]
    ...
    def _run_all_sa_opt_jobs_are_complete(self):
        return all(job.is_complete() for job in self.all_structures_opt_jobs[: self.num_structures_to_opt])
