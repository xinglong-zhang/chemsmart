...
def test_iterate_workflow(
    ...
    try:
        ...
        job_settings = IterateJobSettings(
            config_file=iterate_config_file,
        ...
        # 5. Verify Output
        assert os.path.exists(generated_output_path), "Output file was not generated"
        ...
        for gen, exp in zip(generated_structures, expected_structures):
            # Compare labels
            assert gen['label'] == exp['label'], f"Label mismatch: {gen['label']} != {exp['label']}"
            
            # Compare atom count
            assert len(gen['atoms']) == len(exp['atoms']), \
                f"Atom count mismatch for {gen['label']}: {len(gen['atoms'])} != {len(exp['atoms'])}"
            
            # Compare atom symbols (ensure composition is correct)
            assert gen['atoms'] == exp['atoms'], \
