...
class TestPyMOLJobs:
    ...
    def test_pymol_align_job_on_three_files(
        ...
        style_file = os.path.join(tmpdir, "zhang_group_pymol_style.py")
        pse_file = os.path.join(tmpdir, "alignment_3molecules.pse")
        ...
        out_file = os.path.join(tmpdir, "alignment_3molecules.out")
        ...
        assert "alignment_3molecules.pse" in os.path.basename(pse_file)
    ...
    def test_pymol_nci_job_parameters(
        ...
        assert job_nci_default.label == "benzene_nci"
        assert job_nci_default.nci_basename == "benzene_nci"
        assert job_nci_default.isosurface == 0.5
        ...
        assert job_nci_binary.label == "benzene_nci"
        ...
        assert job_nci_intermediate.label == "benzene_nci"
        ...
        assert job_nci_custom.isosurface == 0.3
        assert job_nci_custom.color_range == 2.0
        assert job_nci_custom.label == "benzene_nci"
