...
import chemsmart.io.pdb.pdbfile as pdbfile_module
...
class TestPDBFile:
    ...
    def test_parse_models_flushes_atoms_seen_before_first_model_record(self):
        """Atoms appearing before any MODEL record must be flushed into
        their own model as soon as the first MODEL record is hit."""
        lines = [
            "HETATM    1  O   HOH A   1       0.000   0.000   0.000"
            "  1.00  0.00           O",
            "MODEL        1",
            "ATOM      1  O   HOH A   2       1.000   0.000   0.000"
            "  1.00  0.00           O",
            "ENDMDL",
        ]
        pdb = PDBFile.__new__(PDBFile)
        pdb.__dict__["raw_lines"] = lines
        models = pdb._parse_models()
        assert len(models) == 2
        assert models[0].residue_numbers == [1]
        assert models[1].residue_numbers == [2]

    # ------------------------------------------------------------------
    # Element inference: dead-code guards around PeriodicTable.to_element
    # ------------------------------------------------------------------
    # PeriodicTable.to_element() never raises for non-empty alphabetic
    # input (it falls back to a best-effort guess instead), so the
    # except/raise branches below are unreachable through real usage.
    # They are exercised here via mocking purely for coverage; see
    # BUGS_FOUND.md #53.

    def test_infer_element_single_letter_lookup_failure_raises(self):
        with patch.object(
            pdbfile_module.p, "to_element", side_effect=Exception("boom")
        ):
            with pytest.raises(ValueError, match="Unable to infer"):
                PDBFile._infer_element_from_atom_name("Q")

    def test_infer_element_two_letter_failure_falls_back_to_single(self):
        real_to_element = pdbfile_module.p.to_element

        def fake_to_element(candidate):
            if len(candidate) == 2:
                raise Exception("no two-letter match")
            return real_to_element(candidate)

        with patch.object(
            pdbfile_module.p, "to_element", side_effect=fake_to_element
        ):
            assert PDBFile._infer_element_from_atom_name("XY") == "X"

    def test_infer_element_two_and_single_letter_failure_raises(self):
        with patch.object(
            pdbfile_module.p, "to_element", side_effect=Exception("boom")
        ):
            with pytest.raises(ValueError, match="Unable to infer"):
                PDBFile._infer_element_from_atom_name("XY")

    # ------------------------------------------------------------------
    # format_pdb_block / _format_pdb_atom_line
    # ------------------------------------------------------------------

    def test_format_pdb_block_preserves_non_atom_lines_with_content(self):
        mol = Molecule(symbols=["O"], positions=np.array([[0.0, 0.0, 0.0]]))
        pdb_block = (
            "REMARK  generated by rdkit\n"
            "HETATM    1  O   MOL A   1       0.000   0.000   0.000"
            "  1.00  0.00           O\n"
            "END\n"
        )
        formatted = PDBFile.format_pdb_block(mol, pdb_block)
        assert "REMARK  generated by rdkit" in formatted

    def test_format_pdb_block_skips_blank_non_atom_lines(self):
        mol = Molecule(symbols=["O"], positions=np.array([[0.0, 0.0, 0.0]]))
        pdb_block = (
            "\n"
            "HETATM    1  O   MOL A   1       0.000   0.000   0.000"
    ...
    def test_format_pdb_block_appends_atoms_missing_from_rdkit_output(self):
        mol = Molecule(
            symbols=["O", "H", "H"],
            positions=np.array(
                [[0.0, 0.0, 0.0], [0.96, 0.0, 0.0], [-0.24, 0.93, 0.0]]
        ...
        # RDKit block only emits the first atom; the writer must
        # append HETATM records for the remaining two.
        pdb_block = (
            "HETATM    1  O   MOL A   1       0.000   0.000   0.000"
            "  1.00  0.00           O\n"
            "END\n"
    ...
    def test_format_pdb_atom_line_hetero_flag_overrides_default_record_name(
        self,
    ):
        mol = Molecule(symbols=["O"], positions=np.array([[0.0, 0.0, 0.0]]))
        mol.info["hetero"] = [True]
        line = PDBFile._format_pdb_atom_line(
            mol, atom_index=0, default_record_name="ATOM"
        )
        assert line.startswith("HETATM")

    def test_format_pdb_atom_line_invalid_record_name_falls_back_to_default(
        self,
    ):
        mol = Molecule(symbols=["O"], positions=np.array([[0.0, 0.0, 0.0]]))
        mol.info["record_type"] = ["FOOBAR"]
        line = PDBFile._format_pdb_atom_line(
            mol, atom_index=0, default_record_name="ATOM"
        )
    ...
    # ------------------------------------------------------------------
    # Atom metadata resolution helpers
    # ------------------------------------------------------------------

    def test_pdb_atom_object_returns_matching_atom(self):
        molecule = SimpleNamespace(atoms=["atom0", "atom1"], num_atoms=2)
        assert PDBFile._pdb_atom_object(molecule, 1) == "atom1"

    def test_pdb_atom_object_returns_none_when_atoms_has_no_len(self):
        molecule = SimpleNamespace(atoms=42, num_atoms=1)
        assert PDBFile._pdb_atom_object(molecule, 0) is None

    def test_pdb_atom_object_returns_none_on_length_mismatch(self):
        molecule = SimpleNamespace(atoms=["atom0"], num_atoms=2)
        assert PDBFile._pdb_atom_object(molecule, 0) is None

    def test_pdb_atom_field_reads_directly_from_atom_object(self):
        atom = SimpleNamespace(atom_name="OW")
        molecule = SimpleNamespace(info={})
        value = PDBFile._pdb_atom_field(
            molecule, atom, 0, ("atom_name", "name")
        )
        assert value == "OW"

    def test_pdb_indexed_value_ndarray_scalar(self):
        assert PDBFile._pdb_indexed_value(np.array(5.0), 0) == 5.0

    def test_pdb_indexed_value_ndarray_in_range(self):
        assert PDBFile._pdb_indexed_value(np.array([1, 2, 3]), 1) == 2

    def test_pdb_indexed_value_ndarray_out_of_range_returns_none(self):
        assert PDBFile._pdb_indexed_value(np.array([1, 2]), 5) is None

    def test_pdb_indexed_value_list_out_of_range_returns_none(self):
        assert PDBFile._pdb_indexed_value([1, 2], 5) is None

    def test_pdb_indexed_value_scalar_fallback(self):
        assert PDBFile._pdb_indexed_value(42, 0) == 42
